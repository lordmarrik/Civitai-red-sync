...
export async function fetchModelLatestVersion (modelId) {
  ...
  // Civitai API still returns civitai.com in downloadUrl after the civitai.red migration
  const redDownloadUrl = downloadUrl.replace('://civitai.com/', '://civitai.red/');
  const summaryMatch = description.match(HTML_FIRST_PARAGRAPH);
  const summary = summaryMatch
    ? summaryMatch[1].replace(/<br\s*\/?>/g, ' ').replace(/<[^>]+>/g, '').trim()
    : '';

  return { id, name, availability, downloadUrl: redDownloadUrl, summary };
}
